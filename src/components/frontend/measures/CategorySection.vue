  ...
  <section class="py-16 bg-white">
    <div class="container mx-auto px-4">
      <div class="text-center mb-12">
        <h2 class="text-4xl font-bold mb-4 text-gray-800">اكتشف المقاييس المناسبة لك</h2>
        <p class="text-xl text-gray-600 max-w-3xl mx-auto">اختر الفئة التي تناسب احتياجاتك للعثور على المقاييس النفسية المعتمدة</p>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        >
          ...
          <!-- عنوان التصنيف -->
          <h3 class="text-xl font-bold text-center mb-4 text-gray-800">{{ category.title }}</h3>
          
          <!-- وصف التصنيف -->
          <p class="text-gray-600 text-center mb-6 text-sm leading-relaxed">{{ category.description }}</p>
          ...
          <div class="flex justify-center gap-6 mb-6 text-sm">
            <div class="flex items-center gap-2 bg-gray-50 px-3 py-2 rounded-lg">
              ...
              <span class="font-semibold text-gray-700">{{ getMeasuresCount(category.id) }} مقياس</span>
            </div>
            <div class="flex items-center gap-2 bg-gray-50 px-3 py-2 rounded-lg">
              <i class="fas fa-clock text-primary-green"></i>
              <span class="font-semibold text-gray-700">{{ getAverageTime(category.id) }} دقيقة</span>
            </div>
          </div>
    ...
    <div class="text-center mt-12">
        <p class="text-xl text-gray-600 max-w-3xl mx-auto">يمكنك البحث عن المقاييس المناسبة  لتلبية احتياجاتك  </p>
      </div>
...
export default {
  ...
  setup(props) {
    ...
    const getActiveCategoryTitle = computed(() => {
      ...
      return category?.title || 'كل المقاييس'
    })
